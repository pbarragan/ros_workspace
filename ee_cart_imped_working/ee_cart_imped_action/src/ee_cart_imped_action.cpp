...
void EECartImpedExecuter::goalCB(GoalHandle gh) {
  ...
  for (size_t i = 0; i < current_traj_.trajectory.size(); i++) {
	...
	return;
      ...
      // STUFF PATRICK ADDED ON 11/4/2014 //

      geometry_msgs::QuaternionStamped origQ, transQ;
      origQ.header.frame_id = currpt.header.frame_id;
      origQ.header.stamp = ros::Time(0);
      origQ.quaternion = currpt.forceDirection;
      transQ.header.frame_id = controller_frame_id_;
      transQ.header.stamp = ros::Time(0);
      try {
	tf_listener.transformQuaternion(controller_frame_id_, origQ, transQ);
      } catch (tf::TransformException ex) {
	ROS_ERROR("ee_cart_imped_action could not transform quaternion.  Error was %s", ex.what());
	return;
      }

      // END STUFF PATRICK ADDED ON 11/4/2014 //
      ...
      transpt.forceDirection = transQ.quaternion;
