...
class Table_and_Objects:
    def __init__(self):
        ...
        f.write('\n\n\nThis is a new Bayesian experiment\n\n\n'+'Initialization:\n')
        f.write('Prior mu: '+str(self.pr_mu)+'\n')
        f.write('Prior var: '+str(self.pr_var)+'\n')
        f.write('Likelihood mu: '+str(self.lh_mu)+'\n')
        f.write('Likelihood var: '+str(self.lh_var)+'\n')
        f.write('Posterior mu: '+str(self.po_mu)+'\n')
        f.write('Posterior var: '+str(self.po_var)+'\n')
        f.write('Model var (measurement + process noise): '+str(self.model_var)+'\n')
        f.write('Initial Force: '+str(self.F)+'\n')
...
def impulse_new(TAO,arm):
 ...
 #first position
    ...
    #let's go back to definitely less than 27 for now
    f_use = min(TAO.F,27)
